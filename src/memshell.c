...
typedef struct
{
	...
	ID id;
	/* adresse du bloc alloue, vaut nullptr si pas de bloc alloue*/
	void *address;
	/* taille du bloc */
	size_t size;
} BLOCINFO;
...
void help()
{
	...
	printf("2) Le shell supporte jusqu'à %ld allocations entre deux initialisations\n", NB_MAX_ALLOC);
}
...
void get_command(char *token, COMMAND *cmd)
{
	...
	if (i < NB_CMD) /* si la commande a ete trouvee */
		...
		*cmd = ERROR; /* sinon on signale l'erreur*/
}
...
ID get_id(void *addr, size_t size)
{
	...
	if (index == NB_MAX_ALLOC)
	{ /* la limite d'allocation est atteinte */
		return 0;
	}
	else
	{
...
void remove_id(ID id)
{
	...
	bloc_info_table[index].address = nullptr;
}
...
int main(int argc, [[maybe_unused]] char *argv[static argc + 1])
{
	...
	while (1)
	{
		...
		case ALLOC:
			...
			if (res == nullptr)
			{
				printf("Erreur : échec de l'allocation (fonction emalloc, retour=nullptr)\n");
			}
			else
			{
				...
				{ /* pas de probleme, affichage de la zone allouée */
